...
La API privada del alumno ahora replica esas validaciones de telefono y emergencia para evitar fichas inconsistentes aunque el endpoint se invoque fuera del frontend.
Las marcas que registra el alumno ahora validan en backend pesos, reps, rondas y tiempos no negativos, y el formulario movil sugiere mejor teclado y rangos base.
...
La ficha `coach-student` ahora destaca faltantes criticos de perfil/salud para que el coach detecte rapido si la ficha del alumno esta lista para seguimiento seguro.
La carga de mediciones desde `coach-student` ahora valida valores no negativos en frontend y backend, y el formulario movil usa inputs numericos con limites minimos.
